...
export default function LeadsTable() {
  ...
  if (!ready) {
    return (
      <div className="rounded-2xl border border-luxury-dark/10 bg-white px-6 py-14 text-center text-sm text-luxury-text/50 shadow-sm">
        Cargando panel…
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <div className="overflow-hidden rounded-2xl border border-luxury-dark/10 bg-white shadow-sm">
        <div className="flex flex-col gap-4 border-b border-luxury-dark/8 bg-luxury-bg/40 px-5 py-4 sm:px-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-sm font-medium text-luxury-dark">
                Embudos de contacto
              </p>
              <p className="text-xs text-luxury-text/50">
        ...
        {visible.length === 0 ? (
          ...
          <>
            {/* Mobile cards */}
            <div className="divide-y divide-luxury-dark/6 md:hidden">
              {visible.map((lead) => (
                <article
                  key={lead.id}
                  role="button"
                  ...
                  className="space-y-3 px-5 py-4 text-left transition hover:bg-luxury-bg/40"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                          ...
                          <span
                            className={`inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[10px] font-medium ${
                              isReviewThisWeek(lead.next_review)
                                ...
                                : "bg-luxury-dark/5 text-luxury-text/55"
                            }`}
                      ...
                      <p className="mt-0.5 text-xs text-luxury-text/50">
                  ...
                  <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
                    <button
                      ...
                      className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-emerald-700/20 bg-emerald-50/80 px-3 py-2 text-xs font-medium text-emerald-800"
                    >
                    ...
                    <button
                      ...
                      className="inline-flex items-center justify-center rounded-lg border border-red-800/15 px-3 py-2 text-red-800/70"
            ...
            <div className="hidden overflow-x-auto md:block">
              <table className="w-full min-w-[760px] text-left text-sm">
                <thead>
                  <tr className="border-b border-luxury-dark/8 bg-luxury-bg/50 text-[11px] uppercase tracking-[0.12em] text-luxury-text/50">
                    <SortHeader
                      label="Paciente"
                    ...
                    />
                    <th className="px-4 py-3.5 font-medium">WhatsApp</th>
                    <SortHeader
                      label="Fecha captura"
                ...
                <tbody>
                  {visible.map((lead) => (
                    <tr
                      key={lead.id}
                      onClick={() => setSelectedId(lead.id)}
                      className="cursor-pointer border-b border-luxury-dark/5 transition-colors last:border-0 hover:bg-luxury-bg/40"
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-start gap-2">
                          <div>
                            ...
                            <p className="mt-0.5 text-xs text-luxury-text/45">
                              {lead.email}
                            </p>
                          </div>
                          {lead.next_review && (
                      ...
                      <td className="px-4 py-4 text-luxury-text/80">
                        {serviceLabel(lead.service)}
                      </td>
                      <td className="px-4 py-4" onClick={(e) => e.stopPropagation()}>
                        <button
                          type="button"
                          onClick={(e) => handleWhatsApp(lead, e)}
                          className="inline-flex items-center gap-1.5 rounded-lg border border-emerald-700/15 bg-emerald-50/60 px-2.5 py-1.5 text-emerald-800 transition hover:bg-emerald-100"
                      ...
                      <td className="whitespace-nowrap px-4 py-4">
                        <p className="text-sm font-medium text-luxury-dark/80">
                          {formatDateShort(lead.created_at)}
                        </p>
                        <p className="mt-0.5 text-[11px] text-luxury-text/45">
                          {new Intl.DateTimeFormat("es-MX", {
                            hour: "2-digit",
                            minute: "2-digit",
                        ...
                        <div className="flex items-center justify-end gap-1.5">
                          {lead.status === "nuevo" ? (
                            <button
                              ...
                              className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-emerald-700/20 bg-emerald-50/70 text-emerald-800 transition hover:bg-emerald-100 disabled:opacity-50"
                              title="Marcar como atendido"
                            >
                              <Check size={16} strokeWidth={2.25} />
                            </button>
                          ) : (
                            <span className="inline-flex h-9 w-9 items-center justify-center text-emerald-700/35">
                              <Check size={16} />
                            </span>
                          )}
                          <button
                            ...
                            onClick={(e) => handleDelete(lead.id, e)}
                            disabled={busyId === lead.id}
                            className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-luxury-dark/10 text-luxury-text/45 transition hover:border-red-700/25 hover:bg-red-50/70 hover:text-red-800 disabled:opacity-50"
                            title="Eliminar lead"
                          >
                            <Trash2 size={15} />
                          </button>
...
function SortHeader({
  ...
  return (
    <th className="px-4 py-3.5 font-medium first:px-6">
      <button
        type="button"
        onClick={(e) => {
          e.stopPropagation();
        ...
        className={`inline-flex items-center gap-1 transition hover:text-luxury-dark ${
          active ? "text-luxury-dark" : ""
        }`}
      >
        {label}
        {active ? (
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide ${
        isPending
          ? "bg-amber-100 text-amber-800 ring-1 ring-amber-700/15"
          : "bg-emerald-100 text-emerald-800 ring-1 ring-emerald-700/15"
      }`}
    >
      <span
...
function PriorityBadge({ priority }: { priority: LeadPriority }) {
  const styles =
    priority === "alta"
      ? "bg-red-50 text-red-800 ring-red-700/15"
      : priority === "media"
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide ring-1 ${styles}`}
    >
      <Flag size={10} />
      ...
      >
        ...
        <div className="flex flex-wrap gap-2 border-t border-luxury-dark/8 px-5 py-4">
          <button
            type="button"
            onClick={onWhatsApp}
            className="inline-flex flex-1 items-center justify-center gap-2 rounded-lg border border-emerald-700/25 bg-emerald-50 px-3 py-2.5 text-sm font-medium text-emerald-900"
          >
